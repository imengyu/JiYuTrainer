...
BOOL KFShutdown()
{
	if (DriverLoaded())
	{
		...
		JTLogInfo(L"DeviceIoControl CTL_SHUTDOWN %s", rs ? L"TRUE" : L"FALSE");
		if (!rs) JTLogError(L"DeviceIoControl CTL_SHUTDOWN %d", GetLastError());

		return rs;
	}
	else JTLogWarn(L"Çý¶¯Î´¼ÓÔØ£¡");
	return false;
}
BOOL KFReboot()
{
	if (DriverLoaded())
	{
		...
		JTLogInfo(L"DeviceIoControl CTL_REBOOT %s", rs ? L"TRUE" : L"FALSE");
		if(!rs) JTLogError(L"DeviceIoControl CTL_REBOOT %d",GetLastError());
	...
	if (DriverLoaded())
	{
		...
		if (DeviceIoControl(hKDrv, CTL_KILL_PROCESS, &pidb, sizeof(ULONG_PTR), &status, sizeof(status), &ReturnLength, NULL))
		{
			if (status == STATUS_SUCCESS)
				return TRUE;
			else JTLogError(L"CTL_KILL_PROCESS ´íÎó£º0x08%X", status);
		}
		else JTLogError(L"DeviceIoControl CTL_KILL_PROCESS ´íÎó£º%d", GetLastError());
...
BOOL KFSendDriverinitParam(bool isXp, bool isWin7) {
	if (DriverLoaded())
	{
		...
		if (DeviceIoControl(hKDrv, CTL_INITPARAM, &pidb, sizeof(pidb), NULL, 0, &ReturnLength, NULL))
			return TRUE;
		else JTLogError(L"DeviceIoControl CTL_INITPARAM ´íÎó£º%d", GetLastError());
	}
	return false;
}
BOOL KFInstallSelfProtect()
{
	if (DriverLoaded())
	{
		...
		if (DeviceIoControl(hKDrv, CTL_INITSELFPROTECT, &pidb, sizeof(pidb), NULL, NULL, &ReturnLength, NULL))
				return TRUE;
		else JTLogError(L"DeviceIoControl CTL_INITSELFPROTECT ´íÎó£º%d", GetLastError());
	}
	return false;
}
